...
export class AuthService {
    ...
    async signUp(email: string, password: string, surname: string, name: string, lastname: string) {
        const user = await this.usersService.findOneByEmail(email);
        if (user) {throw new UnauthorizedException('Почта занята.')};
    
        const salt = await this.passwordService.getSalt();
        const hash = await this.passwordService.getHash(password, salt);
    
        const newUser = await this.usersService.create({email, hash, salt, surname, name, lastname});
    
        const token = await this.jwtService.signAsync({id: newUser.id, email: newUser.email, role: user.role})
  ...
  async signIn(email: string, password: string) {
    const user = await this.usersService.findOneByEmail(email);
    if (!user) {throw new UnauthorizedException('Почта или пароль указаны не верно.')};
